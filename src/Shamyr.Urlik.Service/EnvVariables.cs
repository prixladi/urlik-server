namespace Shamyr.Urlik.Service
{
  public static class EnvVariables
  {
    public const string _AppInsightsKey = "APP_INSIGHTS_KEY";

    public const string _MongoUrl = "MONGO_URL";
    public const string _MongoDatabaseName = "MONGO_DATABASE_NAME";
    
    public const string _RedisHost = "REDIS_HOST";
    public const string _RedisPort = "REDIS_PORT";
    public const string _RedisHitsCapacity = "REDIS_HITS_CAPACITY";
    public const string _RedisHitsTrimInterval = "REDIS_HITS_TRIM_INTERVAL";

    public const string _DefaultUrl = "DEFAULT_URL";

    public const string _AuthorityUrl = "AUTHORITY_URL";
  }
}
